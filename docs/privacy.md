...
Automatic telemetry does not accept or store callsigns, grid locators, QSO records, frequencies, audio, radio models, serial numbers, host names, user names, paths, URLs, tokens, raw IP addresses, user agents, or log content. Alibaba Cloud necessarily processes network metadata while delivering HTTPS requests, but the application does not copy source IPs into product telemetry.

An administrator may separately choose one local log source and time range and manually upload it with optional feedback. This action works independently of automatic telemetry and never runs in the background. Credentials and tokens are redacted again before upload, but diagnostic context can include callsigns, frequencies, network addresses, host information, and local paths. Log objects are private, encrypted, and deleted after 30 days; searchable upload metadata and feedback are retained for up to 365 days.

Telemetry events are stored in Alibaba Cloud Log Service in Hangzhou for 180 days. Installation registration records are retained for up to 3,650 days so aggregate installation counts remain meaningful. Gateway runtime logs are retained for 30 days.
